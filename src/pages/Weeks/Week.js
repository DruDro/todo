...
export class Week extends Component {
  ...
  setTab = (id = moment(this.state.date, 'D-M-YYYY').isoWeekday()) => {
    this.setState({
      ...
      date: moment().isoWeekday(id).week(this.state.week).format('D-M-YYYY')
    });
  ...
  goToDate = (date) => {
      this.setTab();
      this.setState({ date:`/to-dos/${moment(date).format('D-M-YYYY')}` });
  }
  render() {
    return (
      <Tabs
        ...
        {this.days.map(day => {
          const date = moment()
            ...
            .format('D-M-YYYY'),
            tasks = this.state.tasks.filter(task => task.date == date);
          return (
            <Tab key={day.id}>
              <Tablink title={day.title} />
              <TabContent>
                <div
                  className={`tasks-date ${this.state.animation} ${this.props.animation}`}
                >
                  <DatePicker
                    ...
                    <li key={task.id}>
                      ...
                      >
                        ...
                        <input
                          ...
                          onBlur={this.updateTask.bind(this, task, date)}
                          required
                ...
                <AddTask date={date} userId={this.state.userId} getTasks={this.getTasks.bind(this)} />
              </TabContent>
            </Tab>
          )
        })}
